...
def post_data():
    ...
    db = get_db()
    cur = db.execute('INSERT INTO data (envelope, humidity, light, temp_c, temp_f) VALUES (?,?,?,?,?)',[dataNew.get("envelope"), dataNew.get("humidity"), dataNew.get("light"), dataNew.get("tempC"), dataNew.get("tempF")])
    db.commit() 
    return 'data posted'
